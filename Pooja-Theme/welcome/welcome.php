<?php
	if(!class_exists('Uncode_Lite_Welcome')) :

		class Uncode_Lite_Welcome {
			...
			public function __construct() {
				...
				$this->req_plugins = array(
					...
					'instant-demo-importer' => array(
						...
						'name' => __('Instant Demo Importer', 'uncode-lite'),
						...
						'info' => __('Instant Demo Importer Plugin adds the feature to Import the Demo Conent with a single click.', 'uncode-lite'),
					),
				...
				$this->tab_sections = array(
					'getting_started' => __('Getting Started', 'uncode-lite'),
					'recommended_plugins' => __('Recommended Plugins', 'uncode-lite'),
					'support' => __('Support', 'uncode-lite'),
					'free_vs_pro' => __('Free Vs Pro', 'uncode-lite'),
					'more_wp_stuff' => __('More WordPress Stuff', 'uncode-lite'),
				);
				...
				add_action( 'load-themes.php', array( $this, 'uncodelite_activation_admin_notice' ) );

				/* Create a Welcome Page */
				add_action( 'admin_menu', array( $this, 'uncodelite_welcome_register_menu' ) );

				/* Enqueue Styles & Scripts for Welcome Page */
				add_action( 'admin_enqueue_scripts', array( $this, 'uncodelite_welcome_styles_and_scripts' ) );

				/** Plugin Installation Ajax **/
				add_action( 'wp_ajax_uncodelite_plugin_installer', array( $this, 'uncodelite_plugin_installer_callback' ) );

				/** Plugin Installation Ajax **/
				add_action( 'wp_ajax_uncodelite_plugin_offline_installer', array( $this, 'uncodelite_plugin_offline_installer_callback' ) );

				/** Plugin Activation Ajax **/
				add_action( 'wp_ajax_uncodelite_plugin_activation', array( $this, 'uncodelite_plugin_activation_callback' ) );

				/** Plugin Activation Ajax (Offline) **/
				add_action( 'wp_ajax_uncodelite_plugin_offline_activation', array( $this, 'uncodelite_plugin_offline_activation_callback' ) );
			...
			public function get_required_plugin_notification() {
				...
				$uncodelite_plugin_installed_notif = get_option('uncodelite_plugin_installed_notif');
				...
				update_option('uncodelite_plugin_installed_notif', $notif_counter);
				return $notif_counter;
			}

			/** Welcome Message Notification on Theme Activation **/
			public function uncodelite_activation_admin_notice() {
				global $pagenow;
					...
					<div class="notice notice-success is-dismissible">
						<p>
							<?php
								...
								printf( __( 'Welcome! Thank you for choosing %1$s! Please make sure you visit our <a href="%2$s">Welcome page</a> to get started with %1$s.', 'uncode-lite' ), esc_html($this->theme_name), esc_url(admin_url( 'themes.php?page=uncodelite-welcome' ))  );
							?>
						</p>
						<p><a class="button" href="<?php echo esc_url(admin_url( 'themes.php?page=uncodelite-welcome' )); ?>"><?php esc_html_e( 'Lets Get Started', 'uncode-lite' ); ?></a></p>
					</div>
					<?php
				}
			}
			...
			public function uncodelite_welcome_register_menu() {
				$action_count = get_option('uncodelite_plugin_installed_notif');
				$title        = $action_count > 0 ? esc_html__( 'Welcome', 'uncode-lite' ) : esc_html__( 'Welcome', 'uncode-lite' );
				add_theme_page( 'Welcome', $title , 'edit_theme_options', 'uncodelite-welcome', array( $this, 'uncodelite_welcome_screen' ));
			}

			/** Welcome Page **/
			public function uncodelite_welcome_screen() {
				...
				<div class="wrap about-wrap access-wrap">
					<h1>
						<?php
							...
							printf( esc_html__( 'Welcome to %1$s - Version %2$s', 'uncode-lite' ), esc_html($this->theme_name), esc_html($this->theme_version) );
						?>
					</h1>
					<div class="about-text">
						<?php
							/* translators: %s theme name */
							printf( esc_html__( 'The %s is simple, minimal and clean WordPress Theme with elegant design. Though the Theme is simple, it does not lack any features. The theme is loaded with all features required to built a complete website with uncode.', 'uncode-lite' ), esc_html($this->theme_name) );
						?>
					</div>
				...
				<div class="nav-tab-wrapper clearfix">
					<?php foreach($tabs as $id => $label) : ?>
						...
						?>
						<a href="<?php echo esc_url(admin_url('themes.php?page=uncodelite-welcome&section='.$id)); ?>" class="<?php echo esc_attr($nav_class); ?>" >
			...
			public function uncodelite_welcome_styles_and_scripts() {
				...
				wp_localize_script( 'accesspress-basic-welcome-screen', 'uncodeliteWelcomeObject', array(
					'admin_nonce'	=> wp_create_nonce('uncodelite_plugin_installer_nonce'),
					'activate_nonce'	=> wp_create_nonce('uncodelite_plugin_activate_nonce'),
					'ajaxurl'		=> esc_url( admin_url( 'admin-ajax.php' ) ),
					'activate_btn' => __('Activate', 'uncode-lite'),
					'installed_btn' => __('Activated', 'uncode-lite'),
					'demo_installing' => __('Installing Demo', 'uncode-lite'),
					'demo_installed' => __('Demo Installed', 'uncode-lite'),
					'demo_confirm' => __('Are you sure to import demo content ?', 'uncode-lite'),
				) );
			}

			/** Plugin API **/
			public function uncodelite_call_plugin_api( $plugin ) {
			...
			/** Check For Icon **/
			public function uncodelite_check_for_icon( $arr ) {
				if ( ! empty( $arr['svg'] ) ) {
			...
			public function uncodelite_plugin_active($plugin) {
				$folder_name = $plugin['slug'];
				$file_name = $plugin['filename'];
				$status = 'install';
			...
			public function uncodelite_plugin_generate_url($status, $plugin) {
				$folder_name = $plugin['slug'];
				$file_name = $plugin['filename'];
			...
			/* ========== Plugin Installation Ajax =========== */
			public function uncodelite_plugin_installer_callback(){

				if ( ! current_user_can('install_plugins') )
					wp_die( esc_html__( 'Sorry, you are not allowed to install plugins on this site.', 'uncode-lite' ) );
				...
				if (! wp_verify_nonce( $nonce, 'uncodelite_plugin_installer_nonce' ))
					wp_die( esc_html__( 'Error - unable to verify nonce, please try again.', 'uncode-lite') );
				...
				// Get Plugin Info
				$api = $this->uncodelite_call_plugin_api($plugin);
			...
			public function uncodelite_plugin_offline_installer_callback() {
			...
			public function uncodelite_plugin_offline_activation_callback() {
			...
			/** Plugin Activation Ajax **/
			public function uncodelite_plugin_activation_callback(){

				if ( ! current_user_can('install_plugins') )
					wp_die( esc_html__( 'Sorry, you are not allowed to activate plugins on this site.', 'uncode-lite' ) );

				$nonce = isset( $_POST["nonce"] ) ? sanitize_text_field( wp_unslash( $_POST["nonce"] ) ) : '';
				$plugin = isset( $_POST["plugin"] ) ? sanitize_text_field( wp_unslash( $_POST["plugin"] ) ) : '';

				// Check our nonce, if they don't match then bounce!
				if (! wp_verify_nonce( $nonce, 'uncodelite_plugin_activate_nonce' ))
					die( esc_html__( 'Error - unable to verify nonce, please try again.', 'uncode-lite' ) );
				...
				// Get Plugin Info
				$api = $this->uncodelite_call_plugin_api(esc_attr($plugin));
				...
				if($api->name){
					...
					$status = 'failed';
					$msg = esc_html__('There was an error activating $api->name', 'uncode-lite');
				}
			...
			public static function get_plugin_file( $plugin_slug ) {
	      	...
	      	public function get_local_dir_path($plugin) {
	      		...
	      		$url = wp_nonce_url(admin_url('themes.php?page=uncodelite-welcome&section=import_demo'),'uncodelite-file-installation');
	...
	if(class_exists('Instant_Demo_Importer')) :
		...
		$demoimporter->demos = array(
			'uncode-lite' => array(
				'title' => __('Uncode Demo', 'uncode-lite'),
				'name' => 'uncode-lite',
